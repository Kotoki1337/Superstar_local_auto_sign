...
def gen_run(username, password):
    ...
    return result['msg'], result['detail']


def send_groupMsg(str):
    requests.post(f"{cqhttpAddress}/send_group_msg?access_token={access_token}&group_id={group_id}&message={str}")
...
def local_run():
    ...
    for info in USER_INFOS:
        msg, return_re = gen_run(info['username'], info['password'])
        print(f"签到状态：{msg}, {return_re}")
        if msg == 2000:
            pass
        elif msg == 2001:
            for re_list in return_re:
                name = re_list['name']
                date = re_list['date']
                status = re_list['status']
                uid = info['username'][-4:]

                if status == "success":
                    message = f"1***{uid} 的 {name} 于 {date} 在 \"澳門特別行政區\" 完成位置签到"
                elif status == "学生端-签到成功":
                    message = f"1***{uid} 的 {name} 于 {date} 完成自动签到"
                elif status[0] == "学生端-签到成功":
                    message = f"1***{uid} 的 {name} 于 {date} 完成手势签到"
                else:
                    message = f"1***{uid} 的 {name} 于 {date} 完成未知签到，状态：{status}"
                hook.send(message)
                send_groupMsg(message)
        else:
            uid = info['username'][-4:]

            message = f"1***{uid} 抛出错误，{return_re}"
            hook.send(message)
